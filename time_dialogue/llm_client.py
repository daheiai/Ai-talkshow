...
class LLMClient:
    ...
    def _dry_content(self, model_config: ModelConfig, metadata: dict[str, Any]) -> str:
        ...
        if speaker == "old":
            if step_key == "manual_director":
                return (
                    "我先按我能看到的信息判断：这件事在2023年的我看来不会那么快改写格局。"
                    "但如果你们那边已经把成本和信心都打穿了，我最想知道普通人是不是真的感受到了变化。"
                )
            if step_key == "old_bet":
                return (
        ...
        if step_key == "manual_director":
            return (
                "我先回应你刚才那个判断：真正变化不是大家突然相信新东西，而是使用门槛被打低了。"
                "一旦小团队也能直接上手，原来的领先优势就没那么稳了。"
            )
